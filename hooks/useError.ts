...
export function useError(): UseErrorReturn {
  ...
  const { executeWithErrorHandling: executeWithErrorHandlingBase } = useErrorHandling({
  ...
  // Add error to state
  const addError = useCallback((errorData: Omit<PopupError, "id" | "timestamp">) => {
  ...
  // Execute sync operation with error handling
  const executeSyncWithErrorHandling = useCallback(<T>(
    operation: () => T,
    options: {
      errorMessage?: string;
      errorType?: PopupErrorType;
  ...
  const getErrorStats = useCallback(() => {
    const stats = {
      total: errors.length,
      byType: Object.values(PopupErrorType).reduce((acc, type) => {
        acc[type] = 0;
        return acc;
