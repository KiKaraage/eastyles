...
export default defineContentScript({
  matches: ["<all_urls>"],
  main() {
    console.log("[ContentScript] UserCSS content script initializing on:", window.location.href);
      ...
      browser.runtime.onMessage.addListener((message: ContentScriptMessage) => {
        ...
        try {
          if (
            ...
            console.log("[ContentScript] Processing styleUpdate for:", message.styleId);
            contentController.onStyleUpdate(message.styleId, message.style);
          } else if (message.type === "styleRemove" && message.styleId) {
            console.log("[ContentScript] Processing styleRemove for:", message.styleId);
            contentController.onStyleRemove(message.styleId);
          } else if (message.type === "injectFont" && message.fontName && message.css) {
            console.log("[ContentScript] Processing injectFont for:", message.fontName);
            ...
            console.log("[ContentScript] Ignoring unknown message type:", message.type);
          }
        } catch (error) {
  ...
  try {
    ...
    if (cssContent.trim()) {
      ...
      browser.storage.local.set({
        ...
        console.log("Redirecting to Save page with browser storage reference");
        window.location.href = finalUrl;
      }).catch((error: unknown) => {
        console.error("Failed to store in browser storage:", error);
        // Fallback to direct URL for smaller files
        if (cssContent.length < 50000) { // Only for reasonably sized files
...
function injectFontDirectly(fontName: string, css: string): void {
  try {
    ...
    console.log(`[injectFontDirectly] Successfully injected font CSS for ${fontName}`);
  } catch (error) {
    console.error(`[injectFontDirectly] Failed to inject font CSS for ${fontName}:`, error);
  }
}
