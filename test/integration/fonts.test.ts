...
describe("Font UserCSS Round-trip Integration", () => {
  ...
  describe("Complete Font Style Lifecycle", () => {
    it("should complete full round-trip: generate → install → verify → remove", async () => {
      ...
      expect(generatedUserCSS.userCSS).toContain(
      ...
      expect(installation.generatedUserCSS.userCSS).toBe(
        generatedUserCSS.userCSS,
      );

      // Verify it's in the manager
      const retrievedInstallation = manager.getFontStyleInstallation(
        installation.installationId,
      );
      expect(retrievedInstallation).toEqual(installation);

      // Verify font is marked as installed
      expect(manager.isFontInstalled("Instrument Sans", "builtin")).toBe(true);
      expect(manager.isFontInstalled("Instrument Sans", "custom")).toBe(false);

      // Step 3: Verify UserCSS can be parsed back to font application
      const parsedApplication = manager.parseFontFromUserCSS(
        generatedUserCSS.userCSS,
      );
      ...
      const updatedInstallation = await manager.updateFontStyle(
        installation.installationId,
        {
      ...
      expect(updatedInstallation?.fontApplication.targetElements).toBe(
        "h1, h2, h3",
      );
      expect(updatedInstallation?.generatedUserCSS.userCSS).toContain(
        "h1, h2, h3 {",
      );

      // Step 5: Remove the font style
      const removed = await manager.removeFontStyle(
        installation.installationId,
      );
      expect(removed).toBe(true);

      // Verify removal
      const afterRemoval = manager.getFontStyleInstallation(
        installation.installationId,
      );
      expect(afterRemoval).toBeUndefined();
      expect(manager.isFontInstalled("Instrument Sans", "builtin")).toBe(false);
    ...
    it("should handle multiple font styles simultaneously", async () => {
      ...
      const fontNames = installedStyles.map(
        (style) => style.fontApplication.fontName,
      );
      expect(fontNames).toContain("Instrument Sans");
    ...
    it("should handle export and import of font styles", async () => {
      ...
      // Clear manager
      const installationIds = exportedStyles.map(
        (style) => style.installationId,
      );
      await Promise.all(
        installationIds.map((id) => manager.removeFontStyle(id)),
      );
      ...
      const importedFontNames = importedStyles.map(
        (style) => style.fontApplication.fontName,
      );
      expect(importedFontNames).toContain("Instrument Sans");
    ...
    it("should handle font style updates with different configurations", async () => {
      ...
      // Verify final state
      const finalInstallation = manager.getFontStyleInstallation(
        installation.installationId,
      );
      expect(finalInstallation?.fontApplication.targetElements).toBe(
        "article, section",
      );
    });
    ...
    it("should generate valid UserCSS that can be processed by the UserCSS pipeline", async () => {
      ...
      expect(userCSS).toContain("@name        Eastyles Font: Instrument Sans");
      ...
      expect(userCSS).toContain("font-family: 'Instrument Sans'");
      expect(userCSS).toContain("/fonts/Instrument Sans.woff2");
      ...
      expect(parsed?.fontName).toBe("Instrument Sans");
