...
import { UserCSSDomainDetector, DomainDetector } from '../../../services/usercss/domain-detector';
...
describe('DomainDetector', () => {
  ...
  describe('URL normalization', () => {
    it('should normalize URLs correctly', () => {
      ...
      expect(detector.normalizeURL('http://example.com/')).toBe('http://example.com');
      expect(detector.normalizeURL('https://example.com:443/')).toBe('https://example.com');
      expect(detector.normalizeURL('HTTPS://EXAMPLE.COM/PATH')).toBe('https://example.com/PATH');
    });
  ...
  describe('Domain extraction', () => {
    it('should extract domains from URLs', () => {
      ...
      expect(detector.extractDomain('http://sub.example.com:8080/path')).toBe('sub.example.com');
      expect(detector.extractDomain('ftp://ftp.example.com/file')).toBe('ftp.example.com');
    });
  ...
  describe('URL prefix matching', () => {
    it('should match URL prefixes', () => {
      const rules: DomainRule[] = [
        { kind: 'url-prefix', pattern: 'https://example.com/docs', include: true }
      ];

      expect(detector.matches('https://example.com/docs', rules)).toBe(true);
      expect(detector.matches('https://example.com/docs/page1', rules)).toBe(true);
  ...
  describe('Exact URL matching', () => {
    it('should match exact URLs', () => {
      ...
      expect(detector.matches('https://example.com/page?query=1', rules)).toBe(false);
  ...
  describe('Regular expression matching', () => {
    it('should match regular expressions', () => {
      const rules: DomainRule[] = [
        { kind: 'regexp', pattern: 'https://.*\\.example\\.com/.*', include: true }
      ];

      expect(detector.matches('https://sub.example.com/page', rules)).toBe(true);
      expect(detector.matches('https://example.com/page', rules)).toBe(false);
      expect(detector.matches('http://sub.example.com/page', rules)).toBe(false);
    });
  ...
  describe('Multiple rules', () => {
    ...
    it('should handle mixed include/exclude rules', () => {
      ...
      expect(detector.matches('https://sub.example.com', rules)).toBe(true);
      expect(detector.matches('https://blocked.example.com', rules)).toBe(false);
      expect(detector.matches('https://other.com', rules)).toBe(false);
    });
  });
