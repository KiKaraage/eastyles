...
describe('FontSelector', () => {
  ...
  beforeEach(() => {
    ...
    vi.mocked(fontRegistry.getBuiltInFontsByCategory).mockImplementation((category) =>
      mockBuiltInFonts.filter(font => font.category === category)
    );
    vi.mocked(fontRegistry.getFontCategories).mockReturnValue(['sans-serif', 'monospace']);
    ...
    vi.mocked(fontRegistry.generateFontUserCSS).mockReturnValue('/* Generated UserCSS */');
  });
  ...
  it('should allow selecting built-in fonts', () => {
    ...
    const interSample = screen.getByText('Inter').closest('.font-sample');
    fireEvent.click(interSample!);

    expect(interSample).toBeTruthy();
  });
  ...
  it('should handle font check errors gracefully', async () => {
    vi.mocked(fontRegistry.addCustomFont).mockRejectedValue(new Error('Network error'));
