...
describe("VariableControls", () => {
  ...
  const createVariable = (
    overrides: Partial<VariableDescriptor>,
  ): VariableDescriptor => ({
    name: "--test-var",
    type: "text",
  ...
  describe("Rendering", () => {
    it("renders empty state when no variables provided", () => {
      ...
      expect(
        screen.getByText("No configurable variables available"),
      ).toBeTruthy();
    });

    it("renders empty state when variables array is undefined", () => {
      render(
        <VariableControls variables={undefined} onChange={mockOnChange} />,
      );
      expect(
        screen.getByText("No configurable variables available"),
      ).toBeTruthy();
  ...
  describe("CSS Classes and Styling", () => {
    ...
    it("uses daisyUI classes for styling", () => {
      const variables = [
        createVariable({ name: "--test", type: "color", value: "#000000" }),
      ];
      render(
        <VariableControls variables={variables} onChange={mockOnChange} />,
      );
  ...
  describe("Variable Name Display", () => {
    it("removes -- prefix from variable names in labels", () => {
      const variables = [
        createVariable({ name: "--my-variable", type: "text" }),
      ];
      render(
        <VariableControls variables={variables} onChange={mockOnChange} />,
      );
